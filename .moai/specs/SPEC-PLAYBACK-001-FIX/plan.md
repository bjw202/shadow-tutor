...
- [x] 1.0x 속도 설정 시 실제 재생 속도 1.0x 확인
- [x] 1.1x 속도 설정 시 실제 재생 속도 1.1x 확인 (이전: ~1.21x)
- [x] 1.5x 속도 설정 시 실제 재생 속도 1.5x 확인 (이전: ~2.25x)
- [x] 2.0x 속도 설정 시 실제 재생 속도 2.0x 확인 (이전: ~4.0x)
- [x] 0.5x 속도 설정 시 실제 재생 속도 0.5x 확인 (이전: ~0.25x)
- [x] 재생 중 속도 변경 시 즉시 반영 확인
- [x] 속도 변경 시 API 재호출 없음 확인

### 4.2 UI Enhancement 검증

- [x] 프리셋 버튼 6개 렌더링 확인 (0.5, 0.8, 1, 1.2, 1.5, 2)
- [x] 각 프리셋 버튼 클릭 시 해당 속도 적용 확인
- [x] 슬라이더 범위 유지 (0.5~2.0, step 0.05)
- [x] 슬라이더로 세밀한 속도 조절 가능 확인

### 4.3 회귀 테스트

- [x] 재생/일시정지/정지 기능 정상 동작
- [x] 세그먼트 이동 (이전/다음) 정상 동작
- [x] 음성 변경 시 새 TTS 생성 정상 동작
- [x] 볼륨 조절 정상 동작
